...
このゲートを SILS 再確認試験（変更で既存の動作が壊れていないかを自動で確かめる試験）に組み込み、以後のプラント改修の効果と劣化を毎回数値で判定する。
...
- **SILS の原理的限界は SILS では検証できない。** 並行処理の競合（複数タスクが同時に走ることによる競合）や実 WiFi/ESP-NOW の物理層は、SILS の再現性のために本来並行する処理を一本のループにまとめている構造上、原理的に再現できない（旧 RESET_PLAN §11、§10 参照）。実機並行性の検証は別途行う。
...
- **χ² ゲート過剰棄却（2026-06-08, `90093c1`）**: 加速度-姿勢 χ² 検定のしきい値 `accel_att_noise` が 0.06 と厳しすぎ、観測を71%の頻度で棄却していた。0.8 へ緩和して解消。
