...
| 3 | Recalibrate the $C_T$/$C_Q$/thrust_efficiency triple (firmware Ct switch) | The 2026-07-15 thrust-stand measurement of $C_T$ was retracted (2026-08-03) for lacking a valid simultaneous voltage/RPM/thrust measurement on the new propeller. At the time of retraction, the firmware (`actuator.cpp`'s `MOTOR_CT`) already numerically matched the provisional adopted value $C_T=1.00\times10^{-8}$, so this task is now resolved | **Resolved (2026-08-03)** — following the Ct retraction, the firmware value $C_T=1.00\times10^{-8}$ became the adopted value itself, eliminating the firmware/SIL-plant divergence (the static Am/Bm/Cm curve was already dropped on 2026-07-26). Follow-up task: confirm $C_T$/κ via a bench V-ω-T simultaneous co-measurement (pending). Reconsider the need for thrust_efficiency once confirmed |
