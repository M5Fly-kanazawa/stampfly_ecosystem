...
| 3 | $C_T$/$C_Q$/thrust_efficiency の3点セット再較正（ファームCt切替） | 2026-07-15 thrust stand実測の $C_T$ は、新プロペラでの電圧・回転数・推力の有効な同時計測を欠くため撤回（2026-08-03）。撤回時点でファームウェア（`actuator.cpp` の `MOTOR_CT`）は暫定採用値 $C_T=1.00\times10^{-8}$ と数値一致していたため、本タスクは解消した | **解消（2026-08-03）** — Ct撤回によりファーム値 $C_T=1.00\times10^{-8}$ がそのまま採用値になり、firmware/SILSプラント間の乖離は消滅（静的曲線 Am/Bm/Cm は2026-07-26に廃止済み）。後継タスク: ベンチでの V-ω-T 3量同時計測による $C_T$/κ の確定（計測待ち）。確定時は thrust_efficiency の要否も再検討する |
...
| 3 | Recalibrate the $C_T$/$C_Q$/thrust_efficiency triple (firmware Ct switch) | The 2026-07-15 thrust-stand measurement of $C_T$ was retracted (2026-08-03) for lacking a valid simultaneous voltage/RPM/thrust measurement on the new propeller. At the time of retraction, the firmware (`actuator.cpp`'s `MOTOR_CT`) already numerically matched the provisional adopted value $C_T=1.00\times10^{-8}$, so this task is now resolved | **Resolved (2026-08-03)** — following the Ct retraction, the firmware value $C_T=1.00\times10^{-8}$ became the adopted value itself, eliminating the firmware/SILS-plant divergence (the static Am/Bm/Cm curve was already dropped on 2026-07-26). Follow-up task: confirm $C_T$/κ via a bench V-ω-T simultaneous co-measurement (pending). Reconsider the need for thrust_efficiency once confirmed |
