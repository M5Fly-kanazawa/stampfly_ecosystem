...
子機はビーコン受信時に `frame_start_time_us` を親機の送信時刻で上書きすることで、親機と正確に同期します。ビーコンが届かない場合は、自機のタイマーでフォールバック動作します。
...
```
時刻 0ms: タイマー作動、親機がビーコン送信（frame_start_time_us）
         ↓（500μs待機）
