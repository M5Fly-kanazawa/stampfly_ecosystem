...
| `v2026.07.5` | 同上（ファーム無変更） | （未定） | SILS シミュレータの Windows ネイティブ対応（MinGW-w64、WSL不要）・`sf sils regression` と CI 自動回帰判定・GUIインストーラに SILS 開発ツールチェーン導入オプション追加・hover_smoke ハーネス離陸フェーズ修正 |
