...
┌─────────────────────────────────────────────────┐
│  アルゴリズム層 (sf_algo_*)                       │
│  ESKF / PID / 制御配分 / センサ融合 / フィルタ    │
├─────────────────────────────────────────────────┤
│  サービス層 (sf_svc_*)                            │
│  状態管理 / テレメトリ / CLI / ログ / LED / 通信   │
├─────────────────────────────────────────────────┤
│  HAL層 (sf_hal_*)                                │
│  BMI270 / BMP280 / BMM150 / VL53L3CX / PMW3901  │
│  モータ / LED / ブザー / ボタン / 電源            │
└─────────────────────────────────────────────────┘
```
...
```
フライトログ取得 → sf tune gui で解析 → パラメータ調整
    → sf build → sf flash → 検証フライト
```
...
```
  制御設計          実装            検証           解析
┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│ループ整形 │→│ファームウェア│→│シミュレータ│→│ログ解析   │
│Notebook  │  │sf build  │  │sf sim    │  │sf log    │
│Python SDK│  │sf flash  │  │SIL/HIL   │  │sf tune   │
└──────────┘  └──────────┘  └──────────┘  └──────────┘
                                               │
                               パラメータ調整 ←──┘
```
