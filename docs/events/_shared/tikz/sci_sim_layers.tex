...
\begin{tikzpicture}[
  ...
  \node[tier=s3color, right=10mm of S2] (S3) {%
    \begin{minipage}{54mm}\raggedright
      ...
      \tsub{Code/Param Identity}
    \end{minipage}%
  };

  % Rightward arrow chain, directly connecting adjacent box edges
  % (S1.east -> S2.west, S2.east -> S3.west): each arrow starts and ends on
  % a box border (T3/T4 fix — the previous version floated 6mm above the
  % boxes, touching neither; see git log). The 10mm box gap leaves a ~7mm
  % straight trunk before each 3mm arrowhead.
  % 右向き矢印の連鎖。隣接する箱の辺を直接つなぐ（S1.east→S2.west，
  % S2.east→S3.west）。矢印の始点・終点が必ず箱の枠に接する（T3/T4修正:
  % 以前は箱の6mm上を浮いて配線され、どちらの箱にも接していなかった。
  % git log 参照）。箱の間隔10mmのうち矢頭3mmを除いた約7mmが幹になる。
  ...
  % Verification input: Layer 3 (SILS) feeds its log into the gate. S3 is
  % centered directly above GATE's span (both centered on S2's x), so this
  % is a plain vertical drop with a 20mm trunk — no jog needed, no crossing
  % with the S1 return bus (left margin) or the S1-S2-S3 label (above S2).
  % 検証の入力: 層3(SILS)のログがゲートに入る。S3はGATEの範囲の真上
  % （どちらもS2のxを中心とする）にあるため、折れ曲がりなしの垂直線で
  % 幹は20mm。左マージンの層1帰還バスや層1-2-3ラベル（S2の真上）とは
  % 交差しない。
  \draw[garr] (S3.south) -- (GATE.north -| S3.south);
