...
\begin{frame}{物理メカニズム / Physical Mechanism}
  \begin{center}
    \vspace{-2mm}
    \resizebox{0.95\textwidth}{!}{\input{../../_shared/tikz/plant_detail}}
  \end{center}
...
\begin{frame}{$\tau_m$ と $K_m$ の導出 / Deriving $\tau_m$ and $K_m$}
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \end{block}
      \vspace{-0.2em}
      % 2026-08-03 Ct撤回によりkappaを4.10e-3へ再改定(stampfly_physical.yaml)
      {\scriptsize
      \begin{tabular}{lll}
        \hline
        $K_f$       & 0.010\,N/duty & 同定値 \\
        $L$         & 0.023\,m & アーム長 \\
        $\kappa$    & 4.10e-3\,m & トルク推力比 \\
        \hline
  ...
  \centering
  実効プラントゲイン: \; $K = K_m / I$
  \quad ($K_{roll}\!=\!102$,\; $K_{pitch}\!=\!70$,\; $K_{yaw}\!=\!8.0$\,rad/s$^2$)
\end{frame}
...
\begin{frame}{パラメータ一覧 / Parameter Summary}
  \begin{table}
    ...
    \begin{tabular}{llccc}
      ...
      % 2026-08-03 Ct撤回によりkappa再改定(stampfly_physical.yaml)
      ...
      \textbf{プラントゲイン} & $\bm{K = K_m/I}$
        & \textbf{102} & \textbf{70} & \textbf{8.0}\,rad/s$^2$ \\
      \hline
    \end{tabular}
  \end{table}
...
\begin{frame}{P 制御の閉ループ / P Control Closed Loop}
  \begin{center}
    \vspace{-2mm}
    \resizebox{0.85\textwidth}{!}{\input{../../_shared/tikz/closed_loop_model}}
  \end{center}
...
\begin{frame}{ゲイン設計表 / Gain Design Table}
  \begin{table}
    ...
    \begin{tabular}{lcccc}
      ...
      % 2026-08-03 Ct撤回によりkappa再改定(stampfly_physical.yaml): kappa変更でK_yaw 12→8.0
...
// 2026-08-03 Ct撤回によりkappa再改定(stampfly_physical.yaml): K_yaw 12.0f→8.0f
const float tau_m = 0.02f;
const float K_roll  = 102.0f;
const float K_pitch =  70.0f;
const float K_yaw   =   8.0f;
float zeta = 0.7f;
