...
\begin{frame}[fragile]{実習 5: レート P 制御で初飛行}
  ...
  \begin{alertblock}{手順}\footnotesize
    \code{sf lesson switch sci2026:5} $\to$ \code{user\_code.cpp} の TODO を埋める
    （\api{ws::gyro\_x/y/z()}，\api{ws::rc\_roll/pitch/yaw()}）
    $\to$ \code{sf lesson build \&\& sf lesson flash}
  \end{alertblock}

  {\footnotesize 本日は\textbf{見るだけ}: 講師が完成コード（$K_p=0.5$）で飛ばす。
  スティックへの追従とわずかな振動を見る。帰宅後に自分の $K_p$ で飛ばして再現する}
\end{frame}
...
\begin{frame}[fragile]{実習 6: システムモデリング}
  ...
  \begin{alertblock}{手順}\footnotesize
    \code{sf lesson switch sci2026:6} $\to$ \code{Kp\_roll/pitch/yaw} を
    設計式で計算するコードを書く $\to$ \code{sf lesson build \&\& sf lesson flash}
  \end{alertblock}

  {\footnotesize 本日は\textbf{見るだけ}: $\zeta$ を変えた3種類の飛行の違いを見る。
...
\begin{frame}[fragile]{実習 7: システム同定（sf sysid fit）}
  ...
  \begin{alertblock}{手順}\footnotesize
    \code{sf lesson switch sci2026:7} $\to$ $K_p$ を設定し
    \api{ws::set\_rate\_target} を呼ぶ $\to$ \code{sf lesson build \&\& sf lesson flash}
    $\to$ 飛行しながら \code{sf log wifi -o flight.csv} $\to$
    \code{sf sysid fit flight.csv --kp 0.5 --plot}
  \end{alertblock}

  {\footnotesize 本日は\textbf{見るだけ}: 同定結果 $K,\tau_m$ が実習 6 の理論値と
  数\%の誤差で一致することを見る。帰宅後に自分のフライトデータで再現する}
\end{frame}
...
\begin{frame}[fragile]{実習 8: PID 制御}
  ...
  \begin{alertblock}{手順}\footnotesize
    \code{sf lesson switch sci2026:8} $\to$ 軸ごとに I 項・D 項・アンチワインドアップ
    を追加 $\to$ \code{sf lesson build \&\& sf lesson flash}
  \end{alertblock}

  {\footnotesize 本日は\textbf{見るだけ}: 実習 5（P のみ）と比べて定常偏差と
  オーバーシュートがどう変わるかを見る。帰宅後に自分のゲインで再現する}
\end{frame}
...
\begin{frame}{PID 工学形式パラメータ\later}
  ...
  現行 $\kappa=4.10\times10^{-3}$\,m の $K=8.0$ を「2次系設計」の設計式に適用すると
...
\begin{frame}[fragile]{実習 9: 姿勢推定（相補フィルタ）}
  ...
  \begin{alertblock}{手順}\footnotesize
    \code{sf lesson switch sci2026:9} $\to$ 相補フィルタを実装
    $\to$ \code{sf lesson build \&\& sf lesson flash} $\to$
    \code{sf monitor} + Teleplot で \code{cf\_roll} と \code{eskf\_roll} を比較
  \end{alertblock}

  {\footnotesize 本日は\textbf{見るだけ}: 機体を手で傾け，相補フィルタと ESKF の
  推定値が Teleplot 上で近い挙動をすることを見る。帰宅後に $\alpha$ を変えて再現する}
\end{frame}
...
\begin{frame}{デモの期待結果: P と PID のステップ応答}
  ...
  {\tiny SILS（MuJoCo）で \code{workshop\_acro\_step.scn}（ロール $+0.3$ を $0.5$\,s）を実行。
  目標 15.1\,deg/s に対し実習 5（P）はピーク 17.9\,deg/s と $-2.4$\,deg/s のアンダーシュート，
  実習 8（PID）はピーク 13.2\,deg/s で振動なし。角速度は真値ロール角の数値微分}

  \vspace{-0.4em}

  {\tiny 再現: \code{sf sils build --target workshop}；
  \code{sf lesson switch sci2026:5 --solution}；
  \code{touch firmware/workshop/main/user\_code.cpp}；
  \code{sf sils build --target workshop}；
  \code{sf sils scenario simulator/sils/scenarios/workshop\_acro\_step.scn --target workshop}
  （実習8は \code{sci2026:8}）。S5「実習: 自分の PID を SILS で飛ばす」も
  \code{workshop\_acro.scn} で同様}
\end{frame}
