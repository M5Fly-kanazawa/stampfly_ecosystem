...
\begin{frame}{飛ばさずに検証し、1 機から先へ広げる}
  \begin{enumerate}
    \item SILS は実機のファームを無改変で PC 上で動かし，ファームの開発と
...
% ------------------------------------------------------------------
\begin{frame}{SILS: 実機のファームをそのまま PC で動かす}
...
metric duty_max < 0.9 in 6.5 9.0
  ...
  PASS/FAIL と実測値（\code{tilt\_max=11.1}）が出る。しきい値は先に .expect 無しで動かし，実測に余裕を持たせて決める。
...
\begin{frame}{再現性の3原則\later}
  ...
  \begin{table}
    ...
    \begin{tabular}{lp{80mm}}
      ...
      \textbf{原則} & \textbf{内容} \\
      \hline
      Code Identity & SILS は実ファームと同じソースをそのままコンパイルして動く \\
      Parameter Identity & 同じパラメータで動かす \\
...
\begin{frame}{理論とコードの対応表\later}
  \begin{table}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{35mm}>{\raggedright\arraybackslash}p{58mm}>{\raggedright\arraybackslash}p{35mm}}
      ...
      物理パラメータの基準ファイル  & \code{control/\allowbreak models/\allowbreak stampfly\_physical.yaml} & \code{sf params check} \\
  ...
  {\footnotesize 3 つのシミュレータは同じ物理パラメータの基準ファイル（\code{stampfly\_physical.yaml}）を参照し、\code{sf params check} で手動コピーの食い違いを検出する}
\end{frame}
