...
./install.sh          # Windows は install.bat
  ...
  {\scriptsize いずれの方法で導入しても、この後の \code{source setup\_env.sh} $\to$ \code{sf doctor}（このあとの「実習 1」で）確認する}
\end{frame}
...
\begin{frame}{機体の軸定義}
  ...
  \end{block}
  {\footnotesize IMU（ジャイロ・加速度センサ）自体は取り付け方向を持たないが、StampFly では機体への実装時に IMU の軸をそのまま機体の Body 座標系（前方・右・下）として採用している}
\end{frame}
