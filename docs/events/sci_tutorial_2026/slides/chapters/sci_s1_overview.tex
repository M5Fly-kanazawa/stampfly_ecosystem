...
\begin{frame}{提供: 400\,Hz 制御ループ}
  ...
  {\small IMU 割込みを起点に、状態推定 $\to$ 制御 $\to$ アクチュエーションが 2.5\,ms 周期で動く。全コンポーネントが同じ周期を共有する}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}{提供: SILS の仕組み --- 実機のファームを PC で動かす}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}\scriptsize\setlength{\itemsep}{1pt}
        ...
        \item \textbf{OS の代わり}: 仮想時計で動く決定論的な疑似 RTOS と ESP-IDF 互換の代替実装。同じ入力なら毎回同じ結果
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[box, fill=sfblue!15, below=of scn, minimum height=10mm] (fw) {実機と同じファームウェア C++\\（疑似 RTOS ＋ 互換の代替実装の上）};
