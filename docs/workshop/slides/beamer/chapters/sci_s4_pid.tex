...
\begin{frame}{要点3つ / Three Key Points}
  \begin{enumerate}
    ...
    \item P 制御から PID 化し，不完全微分・アンチワインドアップ・積分リセットまで含めた「本物の実装」と学習者コードを対比する
...
\begin{frame}{PID 工学形式パラメータ / Engineering Form}
  ...
  {\footnotesize L8 で使う推奨ゲイン（$K_p$ は前ページのモデルベース設計値，$\eta=0.125$ 共通）:
  Roll $(0.25,\,1.67,\,0.01)$，Pitch $(0.36,\,1.67,\,0.01)$，Yaw $(3.19,\,4.0,\,0.005)$}
  \par\vspace{0.2em}
  {\scriptsize Yaw の $K_p$ はレッスン教材の値ではなく，本ページと同じ $\zeta=0.7$ 設計式を
  $K=8.0$（最新の実測値）に適用した値を掲載する}
\end{frame}
...
d_filt = a*d_filt - b*(err-prev_err);
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{alertblock}{本物: sf\_controller\_pid}\scriptsize
  ...
  \code{sf\_controller\_pid/include/pid.hpp} が本物のコード}
\end{frame}
...
\begin{frame}{発展の予告 / What's Next}
  \begin{itemize}\small
    \item \textbf{オートチューン}: \code{sf sysid rate-fit/rate-tune} と同じアルゴリズムを
          機体上で実行する \api{sf\_autotune}。ステップドサイン励振とプラントフィットを
          オンボードで完結させる
...
\begin{frame}{理論とコードの対応表 / Theory-to-Code Map}
  ...
  数値を単純比較できない点が「模型と本物の違い」を最もよく表す}
\end{frame}
