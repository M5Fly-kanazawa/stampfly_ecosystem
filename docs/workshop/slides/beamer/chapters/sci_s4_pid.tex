...
\begin{frame}{実測パラメータ / Measured Parameters}
  ...
  {\tiny 実習 6 配布コードの $K_{yaw}=19$（旧 $\kappa=9.71\times10^{-3}$\,m 由来）は
  この表の $K=8.0$（現行 $\kappa=4.10\times10^{-3}$\,m 由来）と異なる（次ページで詳述）}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{実習 5 の振り返り: レート P 制御 / Recap}
...
% ------------------------------------------------------------------
\begin{frame}{モデルで見る実習 5 の設計 / Model View}
  \begin{table}
    \centering\small
    \begin{tabular}{lcc}
      \hline
...
\begin{frame}{PID 工学形式パラメータ / Engineering Form}
  ...
  （vehicle 実装が追加する改良点）}
  \par
  {\tiny 実習 6 の配布コードは $\kappa$ 改定前の $K_{yaw}=19$（$K_p=1.34$）のまま。
