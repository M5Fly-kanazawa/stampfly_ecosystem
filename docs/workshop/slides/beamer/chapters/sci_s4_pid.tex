...
% ------------------------------------------------------------------
\begin{frame}[fragile]{実習 5 を振り返る: レート P 制御 / Rate P-Control Recap}
...
% ------------------------------------------------------------------
\begin{frame}{モデルで見る実習 5の設計 / Explaining Exercise 5's Gain}
  \begin{table}
    \centering\small
    \begin{tabular}{lcc}
      \hline
...
\begin{frame}{PID 工学形式パラメータ / Engineering Form}
  ...
  （vehicle 実装が追加する改良点）}
  \par\vspace{0.1em}
  {\scriptsize Yaw の設計式を最新の実測値 $K=8.0$ に適用すると $K_p=3.19$ になり，
  設計式で追い直せる例として挙げる}
\end{frame}
...
sf sysid rate-tune --fit fit.json --wc 25 --pm 60
  ...
  直後に \api{ws::set\_rate\_target(roll,pitch,yaw)} を呼ぶ（実習 7 で追加，Data Stream の
...
% ------------------------------------------------------------------
\begin{frame}{デモの期待結果: P 制御と PID 制御のステップ応答 / Expected Result}
  \begin{center}
    \includegraphics[width=\textwidth, height=0.46\textheight, keepaspectratio]{../../../sci_tutorial/fallback/S4_roll_step_p_vs_pid.png}
  \end{center}
...
\begin{frame}{復習パス / Review Path}
  ...
  \begin{tabular}{lll}
    ...
    \hline
    \code{sf lesson switch sci2026:5} & 実習 5 レート P 制御 & \code{rate\_p\_control.tex} \\
    \code{sf lesson switch sci2026:6} & 実習 6 システムモデリング & \code{system\_modeling.tex} \\
    \code{sf sysid fit}               & 実習 7 システム同定 & \code{system\_identification.tex} \\
    \code{sf lesson switch sci2026:8} & 実習 8 PID 制御      & \code{pid\_control.tex} \\
    \code{sf lesson switch sci2026:9} & 実習 9 姿勢推定      & \code{attitude\_estimation.tex} \\
    \hline
  \end{tabular}}
