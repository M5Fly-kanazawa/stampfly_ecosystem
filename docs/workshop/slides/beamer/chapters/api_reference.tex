...
\begin{frame}{今日のゴール / Today's Goal}
  \begin{block}{目標}
    ws:: API 全 45 関数をカテゴリ別に理解し、各関数の使い方を把握する
  \end{block}

  \vspace{0.5em}

  \begin{itemize}
    \item ws:: API 全 45 関数をカテゴリ別に理解
...
\begin{frame}{ws:: API 概要 / API Overview}
  ...
  \begin{tabular}{llr}
    ...
    Sysid Logging   & 制御目標のロギング（L7）   & 2 \\
    LED             & LED 色制御                 & 3 \\
    Utility         & 時刻・電圧・通信・出力     & 4 \\
    \hline
    \multicolumn{2}{l}{\textbf{合計}} & \textbf{45} \\
    \hline
  \end{tabular}
  }
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{System ID API --- 制御目標ロギング}
  \vspace{0.3em}
  {\small
  \begin{tabular}{lll}
    \hline
    \textbf{関数} & \textbf{単位} & \textbf{説明} \\
    \hline
    \texttt{set\_rate\_target(r, p, y)}  & rad/s & 角速度目標を Data Stream に記録 \\
    \texttt{set\_angle\_target(r, p)}    & rad   & 傾き角目標を Data Stream に記録 \\
    \hline
  \end{tabular}
  }
...
void loop_400Hz(float dt) {
    ...
    // ... compute roll_output from roll_target, gyro_x() ...
  ...
  \vspace{0.2em}
  {\footnotesize \warn{注意:} ロギング専用 --- 制御には無関係。呼ばなくても飛行は動く。
  \texttt{sf sysid fit}（L7）が \texttt{rate\_ref} と \texttt{gyro} から
  $u = K_p(\text{rate\_ref} - \text{gyro})$ を復元する}
\end{frame}
...
\begin{frame}{チェックポイント / Checkpoint}
  \begin{alertblock}{確認事項}
    \begin{itemize}
      \item[$\square$] ws:: API 全 45 関数のカテゴリと役割を把握した
