...
\begin{frame}{層3: SILS / Layer 3: SILS}
  ...
  \begin{itemize}\small
    ...
    \item シナリオファイルは 42 本，うち 33 本が \code{.expect} を伴う。32 本が
          CI 回帰（\code{sils-regression.yml}）の既定対象で，workshop 用の 1 本は手動実行
  \end{itemize}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{学習者コードを SILS で試す / Learner Code in SILS}
...
sf sils scenario .../workshop_acro.scn --target workshop
  \end{sflisting}

  \begin{exampleblock}{現状}\scriptsize
    \code{--target workshop} に対応済み。L5/L8 の模範解答で ARM$\to$離陸$\to$
    ホバー相当の飛行を確認できる。高度ループがないため着陸は DISARM に
    よる降下のみ。\code{sf sils regression} の自動 CI には未組み込み
  \end{exampleblock}
\end{frame}
