...
% ------------------------------------------------------------------
\begin{frame}[fragile]{開発環境の導入 (1/2): PC 側 / Toolchain Setup (1/2)}
  \begin{block}{方法①: GUI インストーラ「StampFly Setup」}
    \footnotesize ターミナル操作なしで導入できる。GitHub の Releases ページから OS 別の実行ファイルをダウンロードして起動し、ウィザードに従うだけで \code{sf} CLI 一式（ESP-IDF 含む）が入る\\
...
./install.sh          # Windows は install.bat
  ...
  {\scriptsize いずれの方法で導入しても、この後の \code{source setup\_env.sh} $\to$ \code{sf doctor}（次ページ）で確認する}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}{開発環境の導入 (2/2): 機体とペアリング / Setup (2/2)}
  \begin{block}{開発環境なしでも書き込みだけはできる}
    \small ブラウザから Web Flasher を開き、公開済みの完成版ファームウェアを機体・コントローラへ直接書き込める\\
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{学習者コードの型とワークフロー / Code \& Workflow}
  \begin{columns}[T]
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{データ取得経路② sf telemetry / Data Path 2}
...
% ------------------------------------------------------------------
\begin{frame}[fragile]{データ取得経路③ ログ取得と可視化 / Data Path 3}
...
\begin{frame}[fragile]{デモ: 傾けて確認 / Demo: Tilt Test}
  ...
  \begin{alertblock}{手順}
    \footnotesize \code{sf lesson switch sci2026:2} $\to$ 前ページのコードを書く $\to$ \code{sf lesson build \&\& sf lesson flash} $\to$ Teleplot で確認
  \end{alertblock}
\end{frame}
