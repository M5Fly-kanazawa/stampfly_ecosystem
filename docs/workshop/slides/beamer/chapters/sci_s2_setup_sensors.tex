...
\begin{frame}[fragile]{データ取得経路③ ログ取得と可視化 / Data Path 3: Log \& Visualize}
  \begin{block}{WiFi 経由で 400\,Hz Data Stream を記録する}
    \small \code{sf log wifi} は機体の WiFi アクセスポイントに接続して受信し、CSV として保存する
  \end{block}

  \begin{lstlisting}[style=sfbash, numbers=none]
sf log wifi -d 30              # 30秒キャプチャ -> CSV自動保存
...
\begin{frame}{理論とコードの対応表 / Theory-to-Code Map}
  \begin{table}
    ...
    \begin{tabular}{lccc}
      ...
      \hline
      角速度 $\omega$ [rad/s]\newline Body FRD & \api{ws::gyro\_x()} & \code{gyro\_x} & \code{sensor.imu} \\
      並進加速度 $a$ [m/s\textsuperscript{2}] & \api{ws::accel\_x()} & \code{accel\_x} & \code{sensor.imu} \\
      \hline
    \end{tabular}
  \end{table}
  ...
  {\small 学習者が呼ぶ \api{ws::} 関数は、内部で \code{sensor.imu} トピックを購読しているだけである（L0 $\to$ L1 の対応）}
\end{frame}
