...
\begin{tikzpicture}[
  ...
  \node[tier=l0color] (L0) at (0, 0) {%
    \begin{minipage}{86mm}\raggedright
      \ttier{l0color}{L0: Sketch API}\quad\tns{l0color}{ws::*}\\[3pt]
  ...
  % Downward arrow chain, offset to the right of the boxes (clear of both
  % box border and text — routed entirely in open space, T4 compliant)
  % 下向き矢印の連鎖。箱の右外側（枠・テキストと重ならない空間）に配線
  \draw[arr] ($(L0.south -| L0.east)+(10mm,0)$) -- ($(L1.north -| L1.east)+(10mm,0)$);
  \draw[arr] ($(L1.south -| L1.east)+(10mm,0)$) -- ($(L2.north -| L2.east)+(10mm,0)$);
  \draw[arr] ($(L2.south -| L2.east)+(10mm,0)$) -- ($(L3.north -| L3.east)+(10mm,0)$);
