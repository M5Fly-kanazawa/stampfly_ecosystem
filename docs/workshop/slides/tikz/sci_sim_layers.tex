...
\begin{tikzpicture}[
  ...
  \coordinate (gateLeft) at ($(GATE.west)+(-8mm,0)$);
  \coordinate (s1Left)   at ($(S1.west)+(-8mm,0)$);
  \draw[gatecolor, line width=1.4pt] (GATE.west) -- (gateLeft) |- (s1Left);
  \draw[garr] (s1Left) -- (S1.west);
  \node[font=\scriptsize, text=gatecolor, align=center, rotate=90]
    at ($(gateLeft)!0.5!(s1Left) + (-3mm,0)$) {実機同定値と突き合わせ};
