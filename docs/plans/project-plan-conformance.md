...
作成: 2026-09-12。状態: **実装済み**（2026-09-12 にオーナーが第 5 章の全問いを推奨案で決定し、同日 Phase 1〜4 を完了。決定の内訳は第 5 章末尾。残る「見直し中」の事項は原典 §15 に記録済み。原典 §15 規則 7 に従い、本文書は次回のアーカイブタグ付与時に削除する）。
...
### 決定（2026-09-12、オーナー「すべて推奨で Phase 4 まで進める」）

| # | 決定 |
|---|------|
| Q1 | 空の置き場 10 か所を一括削除。すべて `.gitkeep` のみで取り出す中身が無いため、削除タグは付けない |
| Q2 | `third_party/` を廃止。外部資産は使う側の隣の `vendor/` に置き、ライセンス全文とバージョンを同梱する規則にする（原典 §13）。plotly のライセンス全文を追加 |
| Q3 | 原典 §8 を実態（sf CLI 中心＋sf 非経由の補助 4 つ）に書き換え、`scripts/` は §2・§12 に載せる。`CLAUDE.md` の「全ツールは sf CLI 経由」は「利用者向けツールは sf コマンドとして公開」に改め、sf 非経由の補助は原典 §8 の列挙に限る |
| Q4 | `ros/` は「構築中」として原典 §2 に載せる（推奨を付けていなかった項目。計画文書 `ros2-integration.md` が計画中のまま生きているため削除は見送り） |
| Q5 | `protocol/tools/check_messages.py` を新設し、`messages.yaml` と `espnow_protocol.hpp` の整合を CI で検査する |
| Q6 | §12（改訂後 §14）の文言を実態に合わせ、「静的解析は導入していない」と明記する |
| Q7 | `examples/education/` は入口の見直しまで保留（原典 §11 にその旨を記載） |
| Q8 | `firmware/workshop/` を「旧アーキテクチャ・廃棄・全面書き換え予定」と明記して原典 §4 に載せる。**2026-09-13 訂正: 廃棄ではなく「やりたいことは変えず現行基盤へアップグレード」に変更し、原典 §4 を修正済み** |

原典は節を並べ直した（新: §9 lib/、§12 scripts/、§13 外部資産の規則、§14 CI、§15 運用規則、§16 まとめ）。旧番号を参照する文書は無いことを `git grep` で確認済み。

Q5 の検査を初めて実行した時点で、`ControlPacket.flags` のビット 4 `POS_MODE`（`CTRL_FLAG_POS_MODE = 0x10`、POS_HOLD 切替に実機で使用中）がヘッダにあり `messages.yaml` に無い不一致を検出した。実装は飛行検証済みのため、仕様側に追記して解消した（`protocol/README.md` のビット一覧も同時に更新）。SSOT を名乗る仕様がすでに実装に遅れていたことの実例であり、検査を CI に置く根拠になった。
