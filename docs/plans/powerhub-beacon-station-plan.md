...
| ビーコン未受信時 | 子機は送信を止めない。起動時刻を起点にした自機だけのタイマーで送り続ける（他機と位相は揃わない） | 同 194〜276 行（`first_beacon_received` を参照しない） |
| ID 0 が外部ビーコンを受けたとき | 何もしない。自機のビーコンも送り続ける（二重ビーコンになる） | 同 157 行の `g_tdma_device_id != 0` 条件 |
