...
`target` は `firmware/` 配下のディレクトリ名をそのまま指定する（動的解決、`sf app list` で一覧表示）。`vehicle` は現行（昇格済み）ファームウェア。
...
`target` is resolved dynamically to a directory name under `firmware/` (see `sf app list` for the full list). `vehicle` is the current (promoted) firmware.
