...
| オプション | 説明 | 既定値 |
|-----------|------|-------|
| `-o, --output` | 出力先。`.sflog.zip` で終わるパスはその zip に書く。既存のディレクトリ（またはパス区切り文字で終わるパス）を渡すと、その中に同名の一式をフォルダとして書く。`.csv`/`.jsonl`/`.bin` など他の拡張子は拒否される（一式が唯一の取得形式。派生ファイルは取得後に `sf log convert --aligned`/`--jsonl` で作る） | 自動生成 `logs/flight_<YYYYMMDD>T<HHMMSS>.sflog.zip` |
