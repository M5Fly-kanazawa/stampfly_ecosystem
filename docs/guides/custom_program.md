...
素直に「誤差の微分」を使うと、目標値（スティック値）が階段状に変化するたびに、誤差も一瞬で大きく変化し、微分が急増してしまう——これを**微分キック**と呼ぶ。ACROのレート目標はスティックの12bit値からそのまま作られるため、目標値は常に細かく階段状に変化しており、誤差の微分をそのまま使うと常にノイズだらけの指令になってしまう。
