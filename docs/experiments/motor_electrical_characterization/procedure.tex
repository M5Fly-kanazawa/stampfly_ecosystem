...
$C_q$ & $4.10\times10^{-11}$ & 空力トルク係数（コーストダウン実測・確定） \\
$Q_f$ & $9.507\times10^{-6}$ & 摩擦トルク項 \\
$C_t$ & $1.00\times10^{-8}$ & 推力係数（暫定採用値，2026-08-03。2026-07-15 thrust stand実測値は新プロペラでの有効な同時計測を欠くため撤回済み——本実験系列とは別のベンチ $V$-$\omega$-$T$ 同時計測で確定させる） \\
$\omega_\mathrm{hover}$ & $\approx 3010\,\mathrm{rad/s}$ & ホバー相当角速度（暫定 $C_t$ に基づく参考値） \\
\bottomrule
\end{tabular}
...
$\approx 60$--$70\,\mathrm{mA}$; $C_q = 4.10\times10^{-11}$ (coast-down measured, confirmed); $Q_f = 9.507\times10^{-6}$;
$C_t = 1.00\times10^{-8}$ (provisional adopted value, 2026-08-03 — the 2026-07-15 thrust-stand figure was retracted for
lacking a valid simultaneous voltage/RPM/thrust measurement on the new propeller; to be confirmed by a separate bench
$V$-$\omega$-$T$ co-measurement); $\omega_\mathrm{hover} \approx 3010\,\mathrm{rad/s}$ (reference value based on the
provisional $C_t$).
