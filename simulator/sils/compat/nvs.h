 ...
 * A fresh run starts empty, so params load their defaults (the SILS passes
 * tuning via --params / topics, not flash).
 *
 * 本体ファームはパラメータ・キャリブを NVS に永続化する。SILS には
 * フラッシュが無いので、NVS は実行中だけ生きるインメモリの map で代用する。
 * 起動時は空なので各パラメータはデフォルトを読む（SILS は調整値を
...
// Value type tag (mirrors ESP-IDF nvs_type_t); the SILS shim only needs the
// type to exist for nvs_find_key()'s signature, not its enumerators.
// 値型タグ（ESP-IDF nvs_type_t 相当）。SILS シムは nvs_find_key() のシグネチャに
...
// --- SILS additions: string get/set (old firmware CLI param store) ----------
