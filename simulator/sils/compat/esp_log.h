 ...
 * SILS's machine-readable results / CSV).
 *
 * SILS は本体ファームを無改変で PC 上にコンパイルする。ESP-IDF の
 * ロギングは stderr に流し、stdout（SILS の機械可読な結果・CSV）を
 * 汚さないようにする。
 */
...
// SILS addition: log levels (old firmware uses esp_log_level_t / esp_log_level_set).
typedef enum {
