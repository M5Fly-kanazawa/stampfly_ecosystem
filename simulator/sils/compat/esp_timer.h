...
/// Monotonic microsecond clock — returns the SILS virtual time
/// 単調マイクロ秒時計 — SILS の仮想時間を返す
int64_t esp_timer_get_time(void);

// -----------------------------------------------------------------------------
// Periodic timer API (subset the firmware uses to pace the 400Hz IMU loop).
// On the SILS these register a deterministic virtual-clock wake source with the
...
/// Dispatch method (only TASK is modeled on the SILS). / ディスパッチ方式。
typedef enum {
    ESP_TIMER_TASK,
    ESP_TIMER_ISR,
