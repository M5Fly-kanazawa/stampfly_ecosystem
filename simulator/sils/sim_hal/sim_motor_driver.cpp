 ...
 * still publishes the actuator_motor topic (which the SILS Plant reads) but skips
 ...
 * を include）。SILS は .cpp だけ差し替える: 本物の motor_driver.hpp をそのまま使い
 * Actuator がリンクできるようにする。init() は意図的に「未初期化」のままにして、
 * Actuator::update() は actuator_motor トピック（SILS Plant が読む）を発行しつつ、
