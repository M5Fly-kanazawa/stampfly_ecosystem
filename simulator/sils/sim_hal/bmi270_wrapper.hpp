 ...
 * @brief SILS replacement for the BMI270 IMU wrapper (sensor seam).
 *        BMI270 IMU ラッパーの SILS 用差し替え（センサ境界）。
 *
 ...
 * SILS build simply puts this header on the include path instead of the HAL.
 *
 * sf_hal_bmi270/include/bmi270_wrapper.hpp のドロップイン: 同じ namespace・
 * クラス・imu_task.cpp が使う公開面。SPI トランザクションの代わりに合成
 * サンプルを返す。本体（imu_task.cpp）は無改変; SILS ビルドが HAL の代わりに
...
// SILS stand-in for the BMI270 wrapper.
// BMI270 ラッパーの SILS 代役。
class BMI270Wrapper {
public:
    // Config is accepted for interface compatibility and ignored on the SILS.
    // skip_bus_init mirrors the HAL field so imu_task.cpp (shared source) sets
    // it identically on both builds; the SILS has no real bus so it is a no-op.
    // Config はインターフェース互換のため受け取り、SILS では無視する。
    // skip_bus_init は HAL と同じフィールドを持たせ、共有ソースの imu_task.cpp が
    // 両ビルドで同一に設定できるようにする(SILS は実バスが無いので無動作)。
    struct Config {
