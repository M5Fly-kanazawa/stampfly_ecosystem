...
/**
 * @file frames.hpp
 * @brief The ONLY coordinate transform in the SILS — MuJoCo ↔ StampFly.
 *        SILS で唯一の座標変換 — MuJoCo ↔ StampFly。
 *
 * Spec & rationale: simulator/sils/docs/coordinate_frames.md. Nowhere else in
 * the SILS performs a coordinate transform (the old SILS broke with 4 scattered
 ...
 *
 * 仕様と理由: simulator/sils/docs/coordinate_frames.md。SILS の他のどこでも
 * 座標変換しない（旧 SILS は座標系不整合4箇所で崩れた）。ファームのドライバが
