 ...
 *
 * The firmware paces its 400Hz IMU loop with an esp_timer periodic. On the SILS,
...
esp_err_t esp_timer_start_periodic(esp_timer_handle_t timer, uint64_t period_us)
{
    ...
    timer->sched_id = sils::rtos::Scheduler::instance().add_periodic(
...
esp_err_t esp_timer_stop(esp_timer_handle_t timer)
{
    ...
    if (timer->sched_id >= 0) {
        sils::rtos::Scheduler::instance().remove_periodic(timer->sched_id);
        timer->sched_id = -1;
    }
    return ESP_OK;
}
