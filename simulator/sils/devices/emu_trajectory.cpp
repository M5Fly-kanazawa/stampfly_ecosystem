 ...
 * estimate columns come from the weak sils_emu_estimate hook (a per-firmware glue
 ...
 * フック sils_emu_estimate から（ファーム固有 glue が上書き可）。列は render_video.py と一致。
 *
...
#include "virtual_board.hpp"  // sils_board_get_motor_duty
...
void sils_emu_estimate(float* alt_est, float* roll_est, float* pitch_est, float* yawcmd)
{
...
extern "C" void sils_emu_traj_open(const char* path)
{
...
extern "C" void sils_emu_traj_sample(double t_s, void* plant_ptr)
{
    ...
    sils::Plant* plant = static_cast<sils::Plant*>(plant_ptr);
    ...
    sils_emu_estimate(&alt_est, &roll_est, &pitch_est, &yawcmd);   // weak: no-op default
...
extern "C" void sils_emu_traj_close(void)
{
    if (g_traj != nullptr) {
        std::fclose(g_traj);
        g_traj = nullptr;
