 ...
 * calling sils_espnow_deliver(). That is the only firmware-agnostic seam for the
 ...
 * スロット）に登録する。別ファイルの仮想送信機が sils_espnow_deliver() を呼べば実
...
esp_err_t sils_espnow_set_recv_cb(esp_now_recv_cb_t cb) { g_recv_cb = cb; return ESP_OK; }
esp_err_t sils_espnow_set_send_cb(esp_now_send_cb_t cb) { g_send_cb = cb; return ESP_OK; }
...
void sils_espnow_deliver(const uint8_t* src_mac, const uint8_t* data, int len)
{
    ...
    sils_emu_record_bytes("espnow", data, len);
