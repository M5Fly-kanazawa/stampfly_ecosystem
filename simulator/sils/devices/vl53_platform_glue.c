 ...
 * supplies the host port, sourced from the SILS virtual clock (deterministic).
 *
 ...
 * ファームが platform.c に足すまで、host port を SILS 仮想時計から供給する（決定論的）。
 *
 * @design simulator/sils/RESET_PLAN.md §3 — SILS surfaces firmware gaps  [--]
 */
...
VL53LX_Error VL53LX_GetTickCount(VL53LX_DEV Dev, uint32_t *ptime_ms)
{
    (void)Dev;
    if (ptime_ms != 0) {
        // SILS virtual clock [us] → milliseconds (monotonic, reproducible).
        // SILS 仮想時計 [us] → ミリ秒（単調・再現可能）。
