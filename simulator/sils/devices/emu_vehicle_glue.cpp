 ...
 * sils_emu_estimate hook; defining it STRONG here makes the recorder log the ESKF
 * estimate (alt/roll/pitch) alongside truth, so est-vs-truth divergence is visible
 * in trajectory.csv and the review video. This is legitimate SILS instrumentation:
 ...
 * 軌跡レコーダは弱い no-op の sils_emu_estimate フックを持つ。ここで強い定義を与えると
 ...
 *
 * @design simulator/sils/RESET_PLAN.md §9 — reproducible review video (estimate overlay)  [--]
 */
...
// SILS_EMU_ESKF_DIAG=<path> dumps the firmware's published ESKF internals (accel
...
// SILS_EMU_ESKF_DIAG=<path> でファーム発行の ESKF 内部（加速度バイアス・ジャイロ
...
std::FILE* diag_file()
{
    if (!g_diag_tried) {
        g_diag_tried = true;
        const char* path = std::getenv("SILS_EMU_ESKF_DIAG");
...
extern "C" void sils_emu_estimate(float* alt_est, float* roll_est,
