 ...
 * firmware's ESP-NOW receive callback via sils_espnow_deliver(). The unmodified
...
#include "scenario_inject.hpp"   // sils::inject_rc, sils::kAdcCentre, sils::kFlagArm
...
//
// KNOWN GAP (a real finding the SILS surfaced): the thrust->duty conversion is
...
void stream(int ms, uint16_t throttle, uint8_t flags)
{
    ...
    for (int i = 0; i < frames; ++i) {
        sils::inject_rc(throttle, sils::kAdcCentre, sils::kAdcCentre, sils::kAdcCentre, flags);
...
// weak sils_virtual_pilot_task symbol (absent for other firmwares → no pilot).
// パイロットタスク。emu_vehicle のみにリンクされ、emu_main_generic が weak シンボル
// 経由で起動する（他ファームでは未定義 → パイロット無し）。
extern "C" void sils_virtual_pilot_task(void* /*arg*/)
{
    ...
    for (int t = sils::kAdcCentre; t <= kHoverThr; t += 45) {   // centre..hover at 20 Hz
        sils::inject_rc((uint16_t)t, sils::kAdcCentre, sils::kAdcCentre, sils::kAdcCentre, sils::kFlagArm);
        vTaskDelay(pdMS_TO_TICKS(50));
    }
    std::printf("[pilot] hold hover throttle (ADC %u)\n", (unsigned)kHoverThr);
    stream(60000, kHoverThr, sils::kFlagArm);          // hold until the run ends
