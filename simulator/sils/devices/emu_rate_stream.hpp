 ...
 * (tools/log_analyzer/rate_sysid.py) run on a SILS-generated log too. That code
 ...
 *
 * Code Identity により、実機ログの同定に使ったのと同じコード（rate_sysid.py）を SILS
...
void sils_emu_rate_open(const char* path);
...
// loaded params AND after any SILS_EMU_PARAMS_FILE override has been applied,
// so the sidecar reflects the gains the run actually flew — the replay in
// `sf sils sysid-gate` needs the SAME gains to reconstruct the loop's torque
// output. No-op if the recorder is not open.
// 実行時の実ゲイン（SSOT params、GUI/env 上書き後）を読み `<path>.gains.json` に書く。
// app_main() の param ロード後、かつ SILS_EMU_PARAMS_FILE 上書き適用後に呼ぶこと —
// sidecar は実際に飛んだゲインを反映する必要がある（sf sils sysid-gate の再生に必須）。
// レコーダ未オープンなら no-op。
void sils_emu_rate_write_gains(void);
