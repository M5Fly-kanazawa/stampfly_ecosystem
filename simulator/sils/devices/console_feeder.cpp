...
void sils_console_set_fd(int fd) { g_cli_fd = fd; }
...
void sils_console_write(const char* bytes, int n)
{
    ...
    if (written > 0) {
        sils_emu_record_bytes("key", reinterpret_cast<const uint8_t*>(bytes), written);
    }
    if (written < n) {
        sils_emu_record_note("key_drop", "pipe never drained — bytes dropped");
