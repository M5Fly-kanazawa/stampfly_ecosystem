...
// SILS: route motor channels (0..3) to the virtual board → Plant. 8-bit full scale.
// SILS: モータ channel(0..3) を仮想ボード→Plant へ。8bit フルスケール。
...
{
    (void)speed_mode;
    sils_board_ledc_set_duty((int)channel, duty, 255u);   // StampFly motors: 8-bit
    return ESP_OK;
}
