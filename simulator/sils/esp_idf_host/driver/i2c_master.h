 ...
 * the virtual board's device model via sils_board_i2c_xfer().
 *
 * シムはステートレスかつヘッダのみのため、ハンドル→アドレスの対応表を持てない。
 * 代わりに I2C デバイスアドレスを不透明ハンドル値に埋め込む（高位に非NULLタグ、
 * 低位10ビットにアドレス）。ハンドルは参照されない（ドライバには不透明）ため安全。
 * 各転送でアドレスを復号し sils_board_i2c_xfer() で仮想ボードのモデルへ振り分ける。
 * ---------------------------------------------------------------------- */
#define SILS_I2C_HANDLE_TAG ((uintptr_t)0x51C20000u)   /* "SIC2", non-null tag */
#define SILS_I2C_ADDR_MASK  ((uintptr_t)0x000003FFu)   /* 10-bit device address */
...
static inline uint16_t sils_i2c_addr_of(i2c_master_dev_handle_t h)
{
    return (uint16_t)((uintptr_t)h & SILS_I2C_ADDR_MASK);
}
                                                  ...
                                                  i2c_master_dev_handle_t* ret_handle)
{
    ...
    if (ret_handle) {
        const uintptr_t addr =
            dev_config ? ((uintptr_t)dev_config->device_address & SILS_I2C_ADDR_MASK) : 0u;
        *ret_handle = (i2c_master_dev_handle_t)(SILS_I2C_HANDLE_TAG | addr);
    }
    return ESP_OK;
}
                                            ...
                                            int xfer_timeout_ms)
{
    ...
    return sils_board_i2c_xfer(sils_i2c_addr_of(dev_handle),
