 ...
 *
 * The two call sites in this codebase (simulator/sils/emu/emu_main*.cpp,
 ...
 *
 * 本リポジトリの2つの呼び出し元（simulator/sils/emu/emu_main*.cpp,
...
// 成功する。失敗もブロックもしない。
static inline int sils_win_fcntl(int fd, int cmd, int arg)
{
    (void)fd; (void)arg;
    return (cmd == F_GETFL) ? O_NONBLOCK : 0;
}
#define fcntl(fd, cmd, arg) sils_win_fcntl((fd), (cmd), (int)(arg))
