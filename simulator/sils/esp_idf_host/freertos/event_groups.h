 ...
 * compiling and linking; SILS flows drive the bits explicitly when needed.
 *
 * イベントグループはタスクがセット/クリア/待機できるビット集合。SILS ホスト
 ...
 * コンパイル・リンク可能になる。必要なら SILS フローが明示的にビットを駆動する。
 */
...
// On the single-token SILS scheduler we return the current bits immediately
...
// 単一トークンの SILS スケジューラではブロックせず現在のビットを即時返し、
