 ...
 *
 * simulator/sils/emu/emu_main*.cpp create a pipe, dup2 its read end onto
...
#if defined(_WIN32) && !defined(SILS_WIN_PIPE_DEFINED)
#define SILS_WIN_PIPE_DEFINED 1
...
static inline int sils_win_pipe(int fds[2])
{
    char name[128];
    static volatile long s_counter = 0;
    long id = InterlockedIncrement(&s_counter);
    wsprintfA(name, "\\\\.\\pipe\\sils_cli_%lu_%ld", GetCurrentProcessId(), id);
...
#define pipe(fds) sils_win_pipe(fds)
...
#endif  // _WIN32 && !SILS_WIN_PIPE_DEFINED
