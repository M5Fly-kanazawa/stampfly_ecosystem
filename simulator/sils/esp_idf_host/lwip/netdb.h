...
/**
 * @file lwip/netdb.h
 * @brief Self-contained INERT name-resolution shim for the SILS (no real DNS).
 *        SILS 用の自己完結 inert 名前解決シム（実 DNS 無し）。
 *
 ...
 * resolver calls run but resolve nothing — consistent with "the SILS has no
...
// Inert: resolve nothing (no DNS on the SILS).
// inert: 何も解決しない（SILS に DNS 無し）。
...
static inline const char* gai_strerror(int e) { (void)e; return "no DNS on SILS"; }
