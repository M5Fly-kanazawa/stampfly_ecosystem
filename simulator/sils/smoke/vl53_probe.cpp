 ...
 *
 * @design simulator/sils/docs/vl53_m2_resume.md §3 Option B — offline gen4 harness
 */
...
#include "vl53_device.hpp"  // sils_vl53 chip model
...
VL53LX_Error VL53LX_WriteMulti(VL53LX_Dev_t* dev, uint16_t index, uint8_t* pdata, uint32_t count)
{
    ...
    sils_vl53::xfer(buf, count + 2, nullptr, 0);
...
VL53LX_Error VL53LX_ReadMulti(VL53LX_Dev_t* dev, uint16_t index, uint8_t* pdata, uint32_t count)
{
    ...
    sils_vl53::xfer(reg, 2, pdata, count);
    return VL53LX_ERROR_NONE;
}
...
int main(int argc, char** argv)
{
    ...
    device.I2cDevAddr = (uint8_t)sils_vl53::ADDR_DEFAULT;

    sils_vl53::set_distance_mm(target_mm);
    ...
    for (int f = 0; f < frames; ++f) {
        ...
        sils_vl53::set_distance_mm(dist_f);
