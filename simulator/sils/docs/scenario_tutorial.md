...
> `lib/sfcli/commands/sils.py` の `_eval_expect`/`_traj_metric` を基準とする。
...
`sf sils scenario` で既存シナリオを実行したことはあるが、自分でシナリオと合否判定を新規に書きたい人。スキーマ（データの構造・形式の定義）を一から読み解く必要はなく、本書と手元の実例（`simulator/sils/scenarios/`）を見比べれば書けるようにする。
...
**注意:** 絶対時刻は「直前イベントの終了時刻より前」を指定するとパースエラーになる（1行ずつ順に作動する単一スレッドのドライバのため、追い越しは許さない）。`rc` の `hold_ms` は保持時間ぶんだけそのイベントの終了時刻を後ろへ押すので、絶対時刻を混ぜるときは前のイベントがいつ終わるかを意識すること。迷ったら `+` を使えば自動的に辻褄が合う。
...
| `handle` | `<carry_alt_m> <place_x> <place_y> <lift_ms> <carry_ms> <place_ms>` | 人が持ち上げて運ぶ動作の模擬（lift→right→carry→place）。`carry_alt_m>0`、各所要時間 `>0` 必須。タイムライン上は瞬時に作動し、その後の disarmed な `rc` 保持が実時間を占有する |
...
`thr` の raw 値（3243=離陸バースト、3176=ほぼホバー）は `stab_flight.scn` から流用した実測値。自分のプロジェクトで別のモータ・機体設定を使う場合は、まず `rc` をスティック中央のまま数秒保持するシナリオを実行し、フライトログ一式の `truth.csv` の高度（NED座標の `pos_z` を反転した値）が概ね一定に留まる `thr` を実測してから合わせ込むこと（「まず実行して実測してから」は手順(4)でも繰り返す）。
...
しきい値をきつくしすぎた例として、`tilt_max < 0.087 in 6.5 7.5`（≈5°。実測11.1°に対して明らかに厳しすぎる）だけを書いた `.expect` で実行すると:
...
しきい値の決め方の基本は「まず `.expect` 無し、または緩い値で1回実行して実測値を見てから、実測に余裕（マージン）を持たせて確定する」— 期待値を先に決め打ちしてから帳尻を合わせない。

## 5. GUI との関係

`sf sils gui`（ブラウザで動く SILS 実験環境）の「シナリオ作成」タブでもイベントを表形式で組んで `.scn` を保存できる（保存先は `scenarios/`、`.scn` のみ）。**`.expect` を作成する画面は無い** — GUI サーバ（`gui/server.py`）は保存済みシナリオを実行するとき、同名の `<name>.expect` が `scenarios/` に存在すればそれを自動適用するだけで、`.expect` の生成・編集機能は持たない。合否判定を作りたいときは本書の手順どおり手書きする。
