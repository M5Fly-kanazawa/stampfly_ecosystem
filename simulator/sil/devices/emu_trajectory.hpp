 ...
 * hover_smoke emits) so `sf sil` can render a 3D + state-graph review video of an
 * UNMODIFIED-firmware emulator run. Firmware-agnostic: the truth pose and motor
 * duty come from the Plant + virtual board (SIL infra). The estimate columns come
 * from a per-firmware weak hook (sil_emu_estimate) — without one they mirror the
 ...
 * ファーム非依存: 真値姿勢とモータ duty は Plant＋仮想ボード（SIL基盤）から。estimate
 * 列はファーム固有の弱フック（sil_emu_estimate）から（無ければ真値を写す）。既定 OFF:
...
void sil_emu_traj_open(const char* path);

// Record one row at virtual time `t_s` [s] from the Plant (opaque sil::Plant*):
