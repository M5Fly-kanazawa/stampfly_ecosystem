 ...
 * (simulator/sil/rtos/). The firmware runs UNMODIFIED against these.
 *
 * 本体タスクが呼ぶタスク/通知/遅延の関数を宣言する。実装は決定論的協調
 * スケジューラ（simulator/sil/rtos/）にある。本体ファームは無改変でこれらに
 * 対して動く。
 *
...
// Opaque task handle (points to a sil::rtos::Task).
// 不透明タスクハンドル（sil::rtos::Task を指す）。
