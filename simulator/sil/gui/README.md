# SIL GUI — ブラウザで触る SIL 実験環境 / Browser-based SIL workbench
...
コマンドラインを叩かずに、ブラウザだけで SIL（ソフトウェアインザループ＝実機ファームをPC上で物理シミュレーションと閉ループ実行）の実験ができる GUI です。`sf sil scenario` で内部的にやっていること——シナリオ作成・パラメータ設定・実行・結果のグラフ化・飛行アニメ——を全部画面で行えます。

### 起動

```bash
source setup_env.sh
sf sil build          # 初回・SIL を変更したとき
sf sil gui            # ブラウザが自動で開く（http://127.0.0.1:8765）
```

オプション: `sf sil gui --port 9000`（ポート変更）、`sf sil gui --no-browser`（自動で開かない）。
...
- 「変更をクリア」で全て既定に戻します。
- 仕組み: 変更は `SIL_EMU_PARAMS_FILE`（`name value` 行）で emu 起動時に `params::set_*` へ適用。

## 3. 仕組み（開発者向け）

| 部品 | ファイル | 役割 |
|------|---------|------|
| CLI | `lib/sfcli/commands/sil.py` `run_gui` | `sf sil gui` でサーバ起動 |
| サーバ | `gui/server.py` | 標準ライブラリのみ。静的配信＋JSON API。`sf sil scenario` を裏で実行しバンドルを読み返す |
...
A browser GUI for the SIL (software-in-the-loop) workbench — author scenarios, set
parameters, run, and see interactive graphs + a live 3D flight animation, all without the
command line. It wraps the same pipeline `sf sil scenario` uses on the CLI.

### Launch

```bash
source setup_env.sh
sf sil build      # first time / after changing the SIL
sf sil gui        # opens the browser at http://127.0.0.1:8765
```
...
- **Parameters**: the "パラメータ" tab — edit any of the 54 firmware params; only the
  changed ones are applied to the run (no rebuild), via `SIL_EMU_PARAMS_FILE`.

## 3. How it works

stdlib-only HTTP server (`gui/server.py`) serving a single-page app; runs shell out to
`sf sil scenario` and the run bundle (trajectory.csv / results.json / events.jsonl) is read
