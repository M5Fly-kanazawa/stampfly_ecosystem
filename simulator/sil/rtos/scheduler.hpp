...
/**
 * @file scheduler.hpp
 * @brief Deterministic cooperative RTOS emulator for the SIL host bench.
 *        SIL ホストベンチ用の決定論的協調 RTOS エミュレータ。
 *
 ...
 * 一瞬に完結する。
 *
 * @design simulator/sil/RESET_PLAN.md §5 — run the real loop unmodified
 * @design simulator/sil/RESET_PLAN.md §7 — host RTOS sim, virtual clock
 * @design simulator/sil/RESET_PLAN.md §11 — single deterministic loop
 */
...
// The cooperative scheduler. One instance per SIL run.
// 協調スケジューラ。SIL 実行ごとに1インスタンス。
class Scheduler {
public:
