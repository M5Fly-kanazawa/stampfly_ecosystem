#!/usr/bin/env python3
"""
debug_viewer_timing.py - Test viewer vs headless timing differences
ビューアとヘッドレスのタイミング差をテスト

Hypothesis: Real-time sync in viewer mode causes control timing issues
仮説: ビューアモードのリアルタイム同期が制御タイミングの問題を引き起こす
"""
...
def main():
    # Parse command line
    mode = sys.argv[1] if len(sys.argv) > 1 else "viewer"
    mode = mode.lower()

    use_viewer = mode != "headless"
    use_realtime = mode == "realtime"

    print("=" * 70)
    print(f"Viewer Timing Test: mode={mode}")
    print(f"Viewer: {'ON' if use_viewer else 'OFF'}")
    print(f"Real-time sync: {'ON' if use_realtime else 'OFF'}")
    print("=" * 70)
    ...
    roll_pid = PID(Kp=9.1e-4, Ti=0.7, Td=0.01, eta=0.125,
                   output_min=-5.2e-3, output_max=5.2e-3)
    pitch_pid = PID(Kp=1.33e-3, Ti=0.7, Td=0.025, eta=0.125,
                    output_min=-5.2e-3, output_max=5.2e-3)
    yaw_pid = PID(Kp=1.77e-3, Ti=0.8, Td=0.01, eta=0.125,
                  output_min=-2.2e-3, output_max=2.2e-3)

    print(f"PID: Roll Kp={9.1e-4:.2e}, Ti=0.7, Td=0.01")
    ...
    def run_viewer_loop():
        nonlocal physics_steps, control_steps, next_control_time, last_print, current_torque

        while scene.viewer.is_alive() and physics_steps * PHYSICS_DT < duration:
            ...
            if use_realtime:
                # Real-time sync: run multiple physics steps to catch up
                while sim_time <= real_time and physics_steps * PHYSICS_DT < duration:
                ...
                # No real-time sync but with proper physics sub-stepping
                # Run RENDER_DT / PHYSICS_DT physics steps per viewer frame
                RENDER_FPS = 30
                RENDER_DT = 1 / RENDER_FPS
                steps_per_frame = int(RENDER_DT / PHYSICS_DT)

                for _ in range(steps_per_frame):
                    # Re-check control timing
                    sim_time = physics_steps * PHYSICS_DT
                    if sim_time >= next_control_time:
                        dofs_vel = drone.get_dofs_velocity()
                        gyro_genesis = np.array([float(dofs_vel[3]), float(dofs_vel[4]), float(dofs_vel[5])])
    ...
    if use_viewer:
        success = run_viewer_loop()
    else:
        success = run_headless_loop()

    final_time = physics_steps * PHYSICS_DT
    result = "PASSED" if success and final_time >= duration - 0.1 else "FAILED (diverged)"
    print(f"\nResult: {result} at t={final_time:.1f}s")


if __name__ == "__main__":
    print("Usage:")
    print("  python debug_viewer_timing.py headless  # No viewer, no real-time sync")
    print("  python debug_viewer_timing.py viewer    # Viewer ON, no real-time sync")
    print("  python debug_viewer_timing.py realtime  # Viewer ON, real-time sync")
    print()
    main()
