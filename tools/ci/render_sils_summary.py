...
# -*- coding: utf-8 -*-
"""
tools/ci/render_sils_summary.py

Render a `sf sils regression --json-out <path>` summary as a GitHub Actions
...
`sf sils regression --json-out <path>` の出力を GitHub Actions のジョブサマリ
（$GITHUB_STEP_SUMMARY への Markdown 追記）として描画する。シナリオごとに
1行（名前/target/PASS-FAIL/所要秒）。.github/workflows/sils-regression.yml が使う。
...
Usage / 使い方:
    python3 tools/ci/render_sils_summary.py <results.json>
...
renders a report, it does not itself gate CI (the `sf sils regression` step
...
def render(results_path: Path) -> str:
    if not results_path.exists():
        return (
            "## SILS scenario regression\n\n"
            "`sf sils regression` produced no results file — the build or the "
    ...
    # produced by a pre-xfail `sf sils regression`.
    ...
    title = f"## SILS scenario regression: {verdict} ({n_pass}/{total} scenarios PASS"
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: render_sils_summary.py <results.json>", file=sys.stderr)
        return 2
