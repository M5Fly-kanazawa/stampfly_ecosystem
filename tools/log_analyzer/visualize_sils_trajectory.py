#!/usr/bin/env python3
"""
visualize_sils_trajectory.py - SILS trajectory.csv Visualization Tool
SILS（Software-in-the-Loop）trajectory.csv 可視化ツール

Visualizes the trajectory.csv produced by `sf sils scenario` runs
(simulator/sils/viz/out_scn_<name>/trajectory.csv): altitude, attitude
(roll/pitch), yaw rate, and motor duty, each with truth overlaid on
estimate/command where applicable. The panel choices mirror
simulator/sils/viz/render_video.py's graph_frame() so the static plot and
the review video agree on what "truth vs estimate" means for this format.

`sf sils scenario` 実行が生成する trajectory.csv を可視化する。高度・姿勢
（roll/pitch）・ヨーレート・モータduty の4段で、該当するものは真値と
推定値/指令値を重ねて描く。パネル選定は simulator/sils/viz/render_video.py の
graph_frame() に合わせてあり、静止画とレビュー動画で「真値 vs 推定」の
意味が食い違わないようにしている。

Usage:
    python visualize_sils_trajectory.py <trajectory.csv> [options]
...
Examples:
    python visualize_sils_trajectory.py out_scn_acro_flight/trajectory.csv
    python visualize_sils_trajectory.py trajectory.csv --save traj.png --no-show
"""
...
# Column set that identifies a SILS trajectory.csv (see run_viz() in
# lib/sfcli/commands/log.py for the format-detection dispatch this backs).
# SILS trajectory.csv を識別する列集合（判定本体は lib/sfcli/commands/log.py の
...
def load_trajectory_csv(filename):
    """Load a SILS trajectory.csv into a DataFrame with a zero-based time column.
    SILS trajectory.csv を読み込み、0始まりの time_s 列を付与する。
    ...
    are in DEGREES (verified against simulator/sils/viz/render_video.py, which
...
def visualize_all(df, filename, save_path=None, show=True):
    """Create the 4-stage SILS trajectory overview: altitude / attitude
    (roll | pitch) / yaw rate / motor duty.
    SILS trajectory の4段プロット: 高度 / 姿勢(roll | pitch) / ヨーレート / モータduty。"""
    fig = plt.figure(figsize=(12, 11))
    fig.suptitle(f'SILS Trajectory: {os.path.basename(filename)}', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize SILS trajectory.csv (sf sils scenario output)',
