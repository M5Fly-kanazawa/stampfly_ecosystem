...
### 言葉遣い（用語辞書）
日本語の言い換え・使わない語・訳語・表記・文体の**正本は `docs/contributing/terminology.md`**（2026-09-13 制定。兄弟リポジトリの辞書を統合したもの）。チャット報告・文書・コミットメッセージを出す前に、同文書の「提出前チェックリスト」を適用する。ユーザーから語の指摘を受けたら、その場で同文書に追記してコミットする（指摘 1 回で以後の全セッションに効く状態を保つ）。用語の**意味**の正本は `docs/guides/glossary.md`。
