/**
 * @file eskf_step_hold_test.cpp
 * @brief Long step hold: 0° → 10° and hold for extended time
 *        長時間ステップ保持: 0° → 10° を長く維持
 */
...
int main() {
    ...
    // 0-2s: hover, 2s-22s: hold 10°
    float t_step = 2.0f;
    float cmd_deg = 10.0f;
    float cmd_rad = cmd_deg * M_PI / 180.0f;
    float t_end = 202.0f;
    int steps = (int)(t_end / dt);

    FILE* csv = fopen("step_hold_10deg.csv","w");
    ...
    fclose(csv);
    fprintf(stderr,"Done → step_hold_10deg.csv\n");
    return 0;
}
