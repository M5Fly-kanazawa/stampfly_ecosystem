...
> ヨートルク＝抗力 $2k_Q\omega_0\,\delta\omega$ ＋反トルク $I_r\dot{\delta\omega}$。CW プロペラで両者**同符号**ゆえ $(1+\tau_z s)$ ＝**左半平面（LHP）＝最小位相零点＝位相リード**（RHP・非最小位相ではない）。roll/pitch は推力差動で反トルク項が無く $G=b e^{-Ls}/(s(T_m s+1))$（零点なし）。
>
> ### 同定の限界（実データで証明）
