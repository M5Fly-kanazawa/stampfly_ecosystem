...
> - **安全判定は yaw のみ T→0 を許容**（`api_task.cpp` 物理境界で `t_lo=(axis==2)?0:0.002`）。roll/pitch は実モータ極~31ms ゆえ T≥2ms を維持。
...
- **反トルク**：モータを**加速する瞬間**に機体が受ける反作用トルク（角運動量保存による）。**∝ 角加速度 $\dot\omega$**。
...
収束とともに抗力トルクへ引き継ぐ。**逆向きの反作用トルクは起きない**。
...
| ヨー $\omega_c$ 上限 | **$\omega_{c,cap} = k_z/\tau_z$（$k_z=0.3$）**。上限で PM=60°/GM≈7 dB。$0.5/\tau_z$ で PM が負になり既存の判定が自動棄却。 |
...
ロール3パラ再確認試験（変更で既存の動作が壊れていないかを自動で確かめる試験）は不変（byte一致）。

> **注意:** SILS のヨープラントは最小位相（零点なし）ゆえ**この退化を再現できない**。SILSは再確認試験のみ、**実機が最終判定**。
